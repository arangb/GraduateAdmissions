...
# This needs to be run in the same folder as gac_dir where all the pdfs are: python3 ../GraduateAdmissions/parse_GAC_names.py > log
gac_dir='/home/aran/GAC21/ALLPS' 
...
fac_names=["Agrawal","BenZvi","Bergstralh","Betti","Bigelow","Blackman","Blok","Bocko","Bodek","Boyd","Cardenas","Cline","Collins","Das","Demina","Dery","Dias","Douglass","Eberly","Ferbel","Forrest","Foster","Franco","Frank","Froula","Gao","Bellido","Ghoshal","Gourdain","Guo","Haefner","Hagen","Helfer","Howell","Jordan","Knight","Knox","Mamajek","Manly","Marshall","McCrory","McFarland","Melissinos","Milonni","Murray","Nakajima","Nichol","Orr","Pipher","Quillen","Rajeev","Ren","Rothberg","Rygg","Savedoff","Schroeder","Sefkow","Seyler","Shapir","Slattery","Sobolewski","Stroud","Tarduno","Teitel","John H. Thomas","Thorndike","Tzeferacos","Vamivakas","Van Horn","Visser","Watson","Wolfs","Wu","Zhang","Zhong"]
...
for filename in sorted(os.listdir(gac_dir)): # order the pdf files alphabetically
    if filename.endswith(".pdf"):
        ...
        for fac in fac_names:
            ...
            (fac in student_name): 
                continue # the above statements are meant to avoid false positives
            if not grep_result == "": 
                print(grep_cmd)
                print(grep_result)
                fac_count[fac]+=1
...
import csv
with open('StudentFacnames.csv', 'w') as csvfile:
    writer = csv.writer(csvfile,delimiter=';')
    for key, value in student_foundfac.items():
        writer.writerow([key, value])
print("Wrote file StudentFacnames.csv with two columns: StudentName FoundFacultyNames")
...
# Sort dictionary by value (in reverse order: those with higher counts first)
fac_count_sorted=dict(sorted(fac_count.items(), key=lambda item: item[1], reverse=True))

# Save to csv file with counts:
with open('grep_list.csv', 'w') as f:
    [f.write('{0},{1}\n'.format(key, value)) for key, value in fac_count_sorted.items()]
