...
for sheet_name in sorted(xls.sheet_names): #['2004']:
    ...
    # Find advisor:
    dfadvisor=df.iloc[:,2:-1].copy() # only look at the columns starting in Fall1, and before the cumulative number
    ...
    newdf.to_csv(sheet_name+'_placement.csv',index=False)


# We can easily concatenate all years into one: 
# cat *_placement.csv > placement_04-11.csv
